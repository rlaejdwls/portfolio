...
static AVSampleFormat get_sample_format(int format) {
	switch (format) {
...
{
	char tsTimeStr[AV_TS_MAX_STRING_SIZE];
	int ret = 0;
	if (pkt.stream_index == video_stream_idx) {
		...
		if (*got_frame) {
			printf("video_frame%s n:%d coded_n:%d pts:%s\n",
				...
				av_ts_make_time_string(tsTimeStr, frame->pts, &video_dec_ctx->time_base));
		...
		if (*got_frame) {
			printf("audio_frame%s n:%d nb_samples:%d pts:%s\n",
				...
				av_ts_make_time_string(tsTimeStr, frame->pts, &audio_dec_ctx->time_base));
			ret = av_samples_alloc(audio_dst_data, &audio_dst_linesize, av_frame_get_channels(frame),
				frame->nb_samples, get_sample_format(frame->format), 1);
			if (ret < 0) {
			...
			* this is required since rawaudio expects non aligned data */
			av_samples_copy(audio_dst_data, frame->data, 0, 0,
				frame->nb_samples, av_frame_get_channels(frame), get_sample_format(frame->format));
...
int Demultiplexer::demuxing(int argc, char * argv[]) {
	int ret = 0, got_frame;
	if (argc != 4) {
